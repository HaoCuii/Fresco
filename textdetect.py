...
if tesseract_path:
    ...
    if os.path.exists(windows_path):
        ...
        raise SystemExit(1)
DPI = 300
MIN_CONF = 80          # 80% for all regular detections
SHAPE_MIN_CONF = 30    # 30% for shapeOCR detections
...
def extract_text_and_create_debug_pdf(input_pdf, output_log, output_text_pdf):
    ...
    for page_num in range(num_pages):
        ...
        shape_results = find_text_in_shapes(cv_image_bgr)
        pass_shape_data = []
            ...
            for idx, item1 in enumerate(word_data):
                if _boxes_overlap(item1['bbox'], item2['bbox']):
                    if item2['conf'] > item1['conf'] or (
                        item2['conf'] == item1['conf'] and len(item2['text']) > len(item1['text'])
                    ):
                        replace_idx = idx
                        print(
                            f"     - Replacing ({item1['source']}) '{item1['text']}' ({item1['conf']}) "
            ...
            for idx, existing_item in enumerate(word_data):
                if _boxes_overlap(existing_item['bbox'], shape_item['bbox']):
                    if shape_item['text'] and shape_item['conf'] > existing_item['conf']:
                        ...
                        print(
                            f"     - Replacing ({old_source}) '{old_text}' ({old_conf}) "
                        ...
                        matched_with_existing = True
                        print(
                            f"     - Adding shape '{shape_item['shape_type']}' to "
                            f"({existing_item['source']}) '{existing_item['text']}'"
                        )
                    break
